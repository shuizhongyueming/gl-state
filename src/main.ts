...
export function createGLStateStack(gl: WebGLRenderingContext, variables?: number[]) {
  ...
  const nvariables = variables.filter((type) => textureTypes.indexOf(type) === -1);
  ...
  return {
    push() {
      ...
      stack.stateStacks = nvariables.map((type) => {
        switch (type) {
    ...
    pop() {
      ...
      const { stateStacks, specialVars, curTex } = stack;
      ...
      if (isNeedSaveTexture) {
        ...
        if (restoreActive && typeof curTex !== 'undefined') {
          gl.activeTexture(curTex);
        }
      }

      nvariables.forEach((type) => {
        const sv = stateStacks.shift();
        switch (type) {
          ...
          case gl.CULL_FACE_MODE:
            return gl.cullFace(sv as number);
